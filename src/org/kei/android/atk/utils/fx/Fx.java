...
public class Fx {
  ...
  public static Drawable resize(final Context context, final Drawable image, final int width, final int height) {
    Bitmap b = ((BitmapDrawable)image).getBitmap();
    Bitmap bitmapResized = Bitmap.createScaledBitmap(b, width, height, false);
    return new BitmapDrawable(context.getResources(), bitmapResized);
}
  
  public static <T extends Activity & IThemeActivity> int getAnimationFromPref(final T owner, final AnimationType at) {
    String value = Tools.getPrefString(owner, KEY_ANIMATIONS, default_animation);
    if(value.equals(ANIMATION_SLIDE_LEFT_TO_RIGHT)) {
      if(at == AnimationType.ENTER_IN) return R.anim.slide_leave_in;
      else if(at == AnimationType.ENTER_OUT) return R.anim.slide_leave_out;
      else if(at == AnimationType.LEAVE_IN) return R.anim.slide_enter_in;
      else return R.anim.slide_enter_out;
    } else if(value.equals(ANIMATION_SLIDE_RIGHT_TO_LEFT)) {
      if(at == AnimationType.ENTER_IN) return R.anim.slide_enter_in;
      else if(at == AnimationType.ENTER_OUT) return R.anim.slide_enter_out;
      else if(at == AnimationType.LEAVE_IN) return R.anim.slide_leave_in;
      else return R.anim.slide_leave_out;
    } else if(value.equals(ANIMATION_FADE)) {
      if(at == AnimationType.ENTER_IN) return R.anim.fade_in;
      else if(at == AnimationType.ENTER_OUT) return R.anim.fade_out;
      else if(at == AnimationType.LEAVE_IN) return R.anim.fade_in;
      else return R.anim.fade_out;
    }
    return 0;
  }

  public static <T extends Activity & IThemeActivity> void updateTransition(final T owner, final boolean in) {
    if (in)
      owner.overridePendingTransition(owner.getAnime(AnimationType.ENTER_IN), owner.getAnime(AnimationType.ENTER_OUT));
    else
      owner.overridePendingTransition(owner.getAnime(AnimationType.LEAVE_IN), owner.getAnime(AnimationType.LEAVE_OUT));
  }
  
  public static boolean switchThemeFromPref(final Activity owner) {
    String value = Tools.getPrefString(owner.getBaseContext(), KEY_THEMES, default_theme);
